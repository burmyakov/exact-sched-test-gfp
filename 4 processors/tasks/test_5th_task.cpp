...
bool find_dominating_state_hpt4(const state& s, const TS& ts, mt4& visitedStates) {
...
bool find_dominating_state_hpt3(const state& s, const TS& ts, mt6& visitedStates) {
    ...
    if (s.p[2] <= ts.P[2] - ts.C[2]) { // s.c[2] == 0
        for (mt6::iterator itr6 = visitedStates.begin(); itr6 != visitedStates.end(); itr6++) {
            if (itr6->first > s.p[2]) break;
                ...
                if (find_dominating_state_hpt4(s, ts, itr6->second)) return true;
            }
        }
    } else { // s.c[2] > 0
    ...
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt8::iterator itr8 = visitedStates.begin(); itr8 != visitedStates.end(); itr8++) {
            if (itr8->first > s.p[1]) break;
                ...
                if (find_dominating_state_hpt3(s, ts, itr8->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
void add_state_to_map(const state& s, const TS& ts, mt11* visitedStates) {
    ...
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[2]][s.p[3]][s.p[4]][s.c[4]] = true;
}
