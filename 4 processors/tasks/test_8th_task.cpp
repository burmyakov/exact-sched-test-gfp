...
bool find_dominating_state_hpt4(const state& s, const TS& ts, mt10& visitedStates) {
    ...
    if (s.p[2] <= ts.P[2] - ts.C[2]) { // s.c[2] == 0
        for (mt12::iterator itr12 = visitedStates.begin(); itr12 != visitedStates.end(); itr12++) {
            if (itr12->first > s.p[2]) break;
                ...
                if (find_dominating_state_hpt4(s, ts, itr12->second)) return true;
            }
        }
    } else { // s.c[2] > 0
...
void add_state_to_map(const state& s, const TS& ts, mt17* visitedStates) {
    ...
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[2]][s.p[3]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]] = true;
}
