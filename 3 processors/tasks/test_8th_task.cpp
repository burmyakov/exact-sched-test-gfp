...
bool find_dominating_state_hpt3(const state& s, const TS& ts, mt12& visitedStates) {
    ...
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt14::iterator itr14 = visitedStates.begin(); itr14 != visitedStates.end(); itr14++) {
            if (itr14->first > s.p[1]) break;
                ...
                if (find_dominating_state_hpt3(s, ts, itr14->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
void add_state_to_map(const state& s, const TS& ts, mt17* visitedStates) {
    ...
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[2]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]] = true;
}
