...
bool find_dominating_state_hpt3(const state& s, const TS& ts, mt24& visitedStates) {
    ...
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt26::iterator itr26 = visitedStates.begin(); itr26 != visitedStates.end(); itr26++) {
            if (itr26->first > s.p[1]) break;
                ...
                if (find_dominating_state_hpt3(s, ts, itr26->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
void add_state_to_map(const state& s, const TS& ts, mt29* visitedStates) {
    ...
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[2]][s.p[13]][s.c[13]][s.p[12]][s.c[12]][s.p[11]][s.c[11]][s.p[10]][s.c[10]][s.p[9]][s.c[9]][s.p[8]][s.c[8]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]] = true;
}
...
bool check_pruning_conditions(state& s, const TS& ts, const unsigned short m) {
