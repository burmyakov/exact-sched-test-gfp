    ...
    if (s.p[1] <= ts.P[1] - ts.C[1]) { // s.c[1] == 0
        for (mt18::iterator itr18 = visitedStates.begin(); itr18 != visitedStates.end(); itr18++) {
            if (itr18->first > s.p[1]) break;
                ...
                if (find_dominating_state_hpt3(s, ts, itr18->second)) return true;
            }
        }
    } else { // s.c[1] > 0
...
void add_state_to_map(const state& s, const TS& ts, mt21* visitedStates) {
    ...
    (*visitedStates)[bs_pj][s.p[0]][s.p[1]][s.p[2]][s.p[9]][s.c[9]][s.p[8]][s.c[8]][s.p[7]][s.c[7]][s.p[6]][s.c[6]][s.p[5]][s.c[5]][s.p[4]][s.c[4]][s.p[3]][s.c[3]] = true;
}
...
bool test_10th_task(const bool verbose_, const uint_fast8_t m_, const TS& ts_, unsigned long int& savedStatesNum_, unsigned long int& visitedStatesNum_) {
